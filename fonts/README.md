...
All byte figures below are for compressed woff2 file sizes. Glyph and codepoint deltas don't reconcile to byte deltas by arithmetic; brotli decides. `test/contract/fonts-budget.contract.ts` measures the shipped files and fails if a figure here drifts out of date.

## carn-sans.woff2

Archivo 2.001, renamed to **Carn Sans**, axis-clamped and subset for Càrn. Self-hosted.
...
- OpenType features kept: `case` (lifts `. - /` to cap alignment), `kern`, `tnum` (tabular numerals for right-aligned columns), `ccmp`, `locl`, `liga`, `frac`, `zero`
- Cap height 686, constant across the whole wght × wdth space (makes the small-caps compensation arithmetic clean)

To regenerate:

```sh
curl -sfL -o Archivo.ttf "https://raw.githubusercontent.com/google/fonts/main/ofl/archivo/Archivo%5Bwdth%2Cwght%5D.ttf"
...
`recalcTimestamp=False` alone isn't enough here. `varLib.instancer` re-stamps `head.modified` itself and has no flag to stop it, so the rename step copies the value back from the pinned upstream `Archivo.ttf`. Without both, two clean runs of this recipe produce different bytes. Same rule as the fixture repo's pinned commit dates.

## carn-mono-400.woff2, carn-mono-500.woff2

IBM Plex Mono 2.3, renamed to **Carn Mono** and subset for Càrn. Self-hosted.

- **8,816 B** at 400 and **8,880 B** at 500, 256 glyphs and 209 mapped codepoints each
- Two static files; Plex Mono ships per weight and has no `wght` axis, and so no `fvar` table and no `name` record above ID 255 for the sans face's rename loop to reach
...
- No `tnum` and no `case`, because Plex Mono has neither. Every digit in a monospace face is already one advance width. `--layout-features+=tnum` would be silently ignored (don't add it)
...
This is accepted and recorded. None of the three reaches the mono face in practice. With `typographer` off, markdown-it generates no dash characters at all. `--` and `---` pass through as ASCII, and only an authored U+2013 or U+2014 renders as one, both of which are in Plex. So all three gaps need someone to type the codepoint deliberately: U+2011 is a non-breaking hyphen almost nobody uses, U+2015 is Greek and European quotation typography, and U+2010 is a character that looks identical to the standard ASCII hyphen.

Carn Sans is itself missing U+2012 and U+201B from the same request, so the two shipped ranges are 212 and 209 against 214 codepoints.

To regenerate:

```sh
curl -sfL -o IBMPlexMono-Regular.ttf "https://raw.githubusercontent.com/google/fonts/main/ofl/ibmplexmono/IBMPlexMono-Regular.ttf"
curl -sfL -o IBMPlexMono-Medium.ttf "https://raw.githubusercontent.com/google/fonts/main/ofl/ibmplexmono/IBMPlexMono-Medium.ttf"
...
`recalcTimestamp=False` is what makes the recipe byte-reproducible. Without it fontTools stamps `head.modified` with the time of the run, and the two committed files come out a few bytes different every time. Same rule as the fixture repo's pinned commit dates.
...
Neither restates copyright or license. Those are IDs 0 and 13, verbatim from upstream. Only the sans is "axis-clamped": Plex Mono ships static, so the mono faces are subset and nothing else.
...
Inside the face there's no extra `@font-face`, no extra files, and no extra requests. The CSS collapses to `font-variant-caps: small-caps` without a `font-family` override. Small caps are currently only used in one style: `.t-item`, `wght` 700 and `wdth` 110. The drawn glyphs can be static with no `gvar` deltas across the axis space.

The only reason to change this is if the added glyphs + lookups ever measure larger on the page than a standalone file would cost on the routes using small caps.
