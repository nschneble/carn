...
export async function checkAuth(
  ...
  if (offered === undefined) return { status: "reject", reason: "no-key" };
  ...
  if (row === null) return { status: "reject", reason: "unknown-key" };

  if (request.signature === undefined) return { status: "probe" };
  ...
  if (stored.verify(request.blob, request.signature, request.hashAlgo) !== true)
