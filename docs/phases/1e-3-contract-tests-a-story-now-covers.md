...
were re-shot in the same commits that touched the views below. Until
they're reviewed, every test below keeps running exactly as it does today.

## What counts as covered, and what doesn't

A Tuffgal baseline freezes two artifacts per action and breakpoint: the
...
so a link's destination is genuinely captured rather than implied by
pixels. It also carries the plain text of every paragraph, term,
definition, and code region on the page. A `<p>`'s wording, a `<dl>`'s
key/value pairs, and a diff's `+`/`−` lines all read back as literal text,
not just roles. A rendered sentence is covered the moment a story's
baseline shows it, word for word.

Five things a baseline still can't do. Every assertion resting on one of
them stays where it is:

- **Read a status code or spot a redirect.** `error-no-directory` shows the
  404 page. It can't show that the response was 404 rather than a 200 or a
...
- **Weigh a page, or count a spawn.** Gzip-5 wire bytes and `git`
  ...
  assertion stays.
- **Verify a computed invariant an eye doesn't check.** A baseline proves
  the page still looks like the approved image. It doesn't prove "no sha
  repeats across the two log pages", because a reviewer approving the first
  capture wouldn't have checked. Those stay too.
- **Carry a DOM attribute the accessibility tree drops.** The snapshot is
  roles, accessible names, `/url:` and plain text, and that's all.
  `tabindex`, `lang`, `id`, `datetime`, `aria-describedby`, class names and
  the document `<title>` never appear in it, in any capture. That's the
...
- **Assert an axe rule.** `axe.contract.ts` runs `wcag2a` through
  `best-practice` across both render paths. A Playwright snapshot isn't an
  audit and doesn't run a rule. So anything whose point is a named axe
  outcome, like focus-order-semantics, aria-required-children, or
  target-size, stays.
...
| 307  | `the branch table names the default branch, and the tag table doesn't`         | `gantry-branches` shows `link "main Default"` beside a bare `link "topic"`; `gantry-tags` carries no `Default` anywhere                                                                                                                                                                                                                                                                                                                                                                                           |
| 323  | `an annotated tag carries the marker, a lightweight one doesn't`               | `gantry-tags`: `v1.1.0` shows as `link "v1.1.0 Annotated"`, `v1.0.0` as bare `link "v1.0.0"`                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 524  | `a path the commit doesn't change is a 404, not a crash`, **partial**                                     | `hit-a-dead-end` → `error-no-change` renders `That commit doesn't change src/nope.ts.` and `Check the path, or read the whole commit.`, with no apology. **The `commitFilePage(...) === null` and `.path === "/404"` assertions stay**: neither is a rendered fact                                                                                                                                                                                                                                             |
...
| 598  | `the heading, the title, and the canonical all name the commit`, **partial**                              | `gantry-commit-inlined` renders `heading "Bump version" [level=1]` and `paragraph: 51173f5` (the short sha). **The `<title>`, the og:url `content=`, and the body-text clause stay**: none of `gantry`'s commits carry a body beyond their subject, so no story ever renders one                                                                                                                                                                                                                |
...
| 177  | `ancestors are links, and the current segment isn't`, **in kind**                                 | Every breadcrumb capture: every ancestor listitem is a `link`, and the last listitem is bare text with no link role. `gantry-tree-nested`'s trail (`Càrn` → `gantry` → `apps` → `web`, then bare `src`) is one example among many                                                                                                                                                                                                                                 |
| 192  | `the index page keeps its own masthead, unchanged`, **partial**                                   | `repo-index`: the banner holds `link "Skip to content"` then `link "Càrn"`, and no `navigation` landmark at all. **The exact masthead markup string stays**: matching HTML byte for byte isn't something a snapshot does                                                                                                                                                                                                                                          |
| 204  | `the breadcrumb doesn't replace the repo page's .vh heading`, **partial**                         | `repo-show`: `heading "linklater" [level=1]` and `navigation "Breadcrumb"` both appear, and the breadcrumb's own list carries no heading role. **The literal `class="vh"` stays**: a class name never reaches the tree                                                                                                                                                                                                                                            |
...
| 82   | `one h1, the list's own label, and no other heading`, **partial**                     | `repo-index`: `heading "Repos" [level=1]` appears exactly once. **The `class="t-item"` and `t-xl`-absence assertions stay**: class names never reach the tree                                                                                                                                                                                                   |
| 90   | `the index lists every repo, with no cap and no show-all`, **in kind, not in regime** | `repo-index` shows exactly five rows for five repos, with no `Show all` anywhere. **The 40- and 200-repo cases stay unreached**: the real fixture never grows past five, so the claim the test exists to prove, that the list still doesn't cap at scale, isn't demonstrated                                                                                    |
...
- **`page-csp.contract.ts`** loads synthetic documents (`/marked`,
  ...
  serves is a route Tuffgal captures. And none of what it asserts reaches
  the accessibility tree either way: font metrics, CSS custom-property
  values, refused-resource console text.
...
The rest of `test/contract/` was scanned test by test: `access`,
`assets`, `escaping`,
...
`token-resolution`, `unquoted-attribute`, and `wordmark`. None renders a
full page a story navigates to; they assert on git
