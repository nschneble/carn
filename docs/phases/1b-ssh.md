...
if they fail. `ssh2` falls back to pure JS, and that fallback is what
Phase 0 exercised. Don't add a build toolchain to make them succeed, and
don't
...
- Pushing to a name that doesn't exist creates the repo
- Writes are authorized; reads over SSH aren't gated, since everything is
  public
...
- `carn repo rename`. The plan puts rename in Phase 1, but there's no
...
- **The init migration's `require-lock-timeout` /
  `require-statement-timeout` ignore doesn't carry forward.** Its
...
in this phase, so don't anticipate it.

While you're in `config.ts`: `Number(read('PORT', '3000'))` yields `NaN`
...
not a case to handle; it's a case that can't be expressed. Do not also
write a `..` check, which would imply the regex isn't trusted.
...
and say the repo doesn't exist. Never create on read.
...
The reverse, a row pointing at nothing, isn't.
...
And Phase 0's third gotcha, which isn't in CLAUDE.md and cost the most
...
fully succeeded. If you see that symptom, it's this and nothing else.
...
10. A key with no `ssh_keys` row is rejected, and ssh exits 255
11. A user with a valid key but no write grant, pushing to a repo owned by
    ...
    setup; don't add a migration for it
12. `git-upload-pack` against a name with no row fails and creates
    nothing. Assert no row and no directory afterward.
...
proves authorization exists rather than assuming it, and it's easy to
...
If something here is wrong about the world, like an `ssh2` API that has
moved or a Prisma method that doesn't exist, **stop and say so.** Five of
1a's six
