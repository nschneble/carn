...
That is the whole phase. It is small on purpose: it is the last thing
...
There is no form, and there will not be one in this phase. The web surface
is read-only by architecture — no sessions, no passwords, no per-user
rendering — and adding a write path with authentication would contradict
...
Anything else is refused. That regex is a security boundary — it is what
...
is not a test to fix until it goes green — it is a deliberate three-site
...
holding a `write` grant, and a collaborator who can push should not be able
to change the repo's public URL out from under every link to it.

Add a predicate beside it — `mayAdminister`, or whatever reads better —
that is true for the owner, for `user.isAdmin`, and for a grant at level
...
`repos.owner_id` is immutable per §05. Renaming does not touch it.
...
the on-disk path before and after and assert it is byte-identical, then
...
deliberate; do not invent an audit log here.
...
That is what PLAN specifies and it is the right call for a one-person forge
where the owner is the only one renaming anything. It is worth stating in
the brief because it is the kind of decision that looks like an oversight
later: a name-history table and a 301 from retired names is a real feature,
and it is a different phase if it is ever wanted.
...
**Redirect every ref, including a non-default one**, and do not special-case
the default branch.

The objection to that is obvious: `/r/gantry/tree/v1.2.0/` means "the root
...
the ref while you climb — `api` to `apps` stays on `v1.2.0` — and drops it
the moment you click `gantry`. The 301 makes the URL bar agree with what
the navigation has always done.

The 404 is not protecting anything either. Nothing in the product links to
...
is not set on the Fastify instance (`src/app.ts:17`), so it defaults to
...
**The no-slash form is the one a person is more likely to type**, and it is
currently the one the redirect would not cover. Fix both. Register the bare
...
URL today, and a forge whose pages are cached forever should not grow a
...
- **The repo-page design pass** — footer padding, folder-slash spacing, the
  ...
  1e merges, and it is Nick's, not this one's.
...
9. `/r/:repo/tree/main` — no trailing slash — answers the same 301, not a
   404.
