...
> dependency, `@prisma/adapter-pg` (`pg` arrives transitively, so don't add
> it to `package.json` unless you import it directly, which you shouldn't
...
> used bare `DateTime`, which Prisma maps to `TIMESTAMP(3)`, with no time
> zone.
...
> somewhere to replay it into. Don't add one. Prisma 7 removed the
...
> not true of this project, raising more findings rather than fewer. Keep
> the
...
> premise silenced 24 findings. That's configuration used as concealed
> exclusion,
...
>
> 2. **If that doesn't work, make the config honest.** Set
...
stack, the naming registers, and the verified gotchas. This brief doesn't
...
it's the one piece of this codebase where a mistake is a vulnerability
rather than a bug.
...
- **A reserved-name list.** The `/r/` prefix makes route collisions
  structurally impossible. PLAN.md §06 is explicit that there's no list to
  maintain. Don't add one.
- Sessions, cookies, CSRF, or any auth middleware. There's no auth.
- Rate limiting
...
Decisions, not options. Don't substitute.
...
That's the entire list for 1a. CLAUDE.md's rule applies: **stop and ask**
...
Don't also leave a plain `@unique` on `name` in the schema. It would
permit `Foo` and `foo` to coexist, which the URL space can't represent.
...
Because the schema now contains state Prisma didn't generate, add a short
...
with no keys simply can't authenticate yet, which is correct for 1a.
...
A rule that fires because squawk is missing an environmental fact isn't a
false positive. It's a correct inference from wrong inputs, and the fix is
...
project, which isn't what Prisma's documentation implies and has changed
...
config fact that's false is worse than no config file at all: it silences
findings on a premise that doesn't hold, which is the exact failure this
...
**1 · Lifecycle artifact.** The rule guards against a hazard that applies
...
`prefer-bigint-over-int` on `repos.next_number` is this case: it's a
per-repo counter of issues plus PRs, against an `int4` ceiling of
2,147,483,647, on a single-person forge. Keep `Int`.

**3 · Correctness.** The rule is catching something that's wrong at row
one, not at row N. A scale that never arrives doesn't rescue it.
...
all of `events` as naive timestamps in later phases, and the finding would
...
Escape exactly these five, and do it in one pass so `&` can't be
double-encoded: `&` → `&amp;`, `<` → `&lt;`, `>` → `&gt;`, `"` → `&quot;`,
`'` → `&#39;`.
...
for attribute values; don't "optimize" either away.
...
> position. It doesn't, and no five-character set could. An unquoted value
...
> a quote mark doesn't already buy. No mainstream templating system does
> it that way either.
>
...
- a nested `html` result isn't double-escaped
...
Read it once at startup into a frozen object; don't touch `process.env`
elsewhere. `prisma.config.ts` is the one exception, since the CLI loads it
outside the app. Fastify's built-in pino covers logging; don't add one.
...
adapter owns connection pooling, so don't construct a `Pool` yourself.
...
language can't express. Whether `migrate diff` reports them as drift
depends on whether its engine models those object types at all, and that's
...
- **Diff contains anything else.** Real drift. Fix the schema or the
  migration; don't adjust the check.

Report which case occurred in your handoff notes.

Check 7 is the one worth being careful about. It's easy to write a
...
- Any exit check you couldn't make mechanical, and why
- The squawk rules excluded, if any, and the justification for each

If something here turns out to be wrong, like a Prisma API that doesn't
exist or a squawk rule that can't be excluded narrowly, **stop and say so
rather
