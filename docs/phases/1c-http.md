...
- `/r/:repo/archive/:rev.tar.gz`, the tightest rate-limit zone in the whole
  design, and nothing needs it yet
...
- **The init migration's `require-lock-timeout` /
  `require-statement-timeout` ignore doesn't carry forward**
...
- **`resolveRepo(target)` from `src/repos/resolve.ts`** does name
  validation and the `lower(name)` lookup. Don't write a second validator,
  and don't join a name into a path.
- Mirror `exec.ts`'s child lifecycle: an `AbortController` aborted on
  client disconnect, passed as `signal`. On HTTP that's
  ...
  the response. And `req.destroyed` is true whether the client left or not,
  so it can't tell the two apart. This matters more here than over SSH. A
  browser or crawler abandoning a clone is routine.

## The three corrections

CLAUDE.md's *Smart HTTP* section states these. They're the whole reason
this phase is more than an afternoon.

**1 · The service header.** `git upload-pack --advertise-refs` doesn't emit
it. Under protocol v0 you prepend it yourself:
...
for the line. Under `Git-Protocol: version=2` there's **no** service
...
the `Git-Protocol` request header. Pass the header value through; don't
synthesize it.
...
5. Under v2 the response does **not** carry the service header. That's the
   corruption case, so assert it rather than assuming it.
...
If something here is wrong about the world, like a Fastify API that has
moved or a pkt-line length that's off by one, **stop and say so.** Five of
1a's six amendments and several of 1b's were defects in the brief rather
than the code. Surfacing them early is the process working.
