...
>
> Use `rehype-sanitize` or `sanitize-html`. Both scored zero real leaks in testing. **Don't use DOMPurify with linkedom.** It's widely recommended online as a fast jsdom alternative, and it _silently does nothing_. Feature detection fails, `isSupported` is falsy, and `sanitize()` returns its input unchanged without any errors. If you do use DOMPurify server-side, assert `DOMPurify.isSupported === true` at startup and budget for jsdom's 122 MB RSS and 600 ms init.
...
| `/r/:repo/asset/:rev/*`           | Web   | A README's relative image, addressed by path. Revalidates on the OID.         |
| `/r/:repo/blob-asset/:asset`      | Web   | An image blob inline in the blob view, by OID. Immutable.                     |
