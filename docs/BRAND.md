...
**Table layouts are always `fixed`, never `auto`.** Specify columns as fixed or percentage widths. For example, age is 46px, name is 65%, and subject is the rest. Name isn't `calc((100% - 46px) * 0.7)`. Yes, this means that it's precise width will vary slightly with screen width, and that's okay.
