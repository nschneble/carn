...
- **Hit area** — Two shapes, and how many links a row holds decides which. The commit log and the branch and tag lists put a link in all three cells, so each cell's own link fills its own cell: three separately focusable targets, covering the row end to end, and an overlay would swallow two of them. The file tree and the repo index hold one link, the name — their subject and age are plain text — so cell-fills-cell would cover under half the row, and the name's link takes an `::after` stretched over the whole `<tr>` instead. The cost is that the subject and the age stop being selectable on those two views; the row washes `--sunk` on hover and focus either way, and the width it washes is the width that takes a click. **Live everywhere a row leads somewhere**, which 1e made true of the file tree — file rows link to `/r/:repo/blob/:rev/*`, directory rows to `/r/:repo/tree/:rev/*`. A gitlink row is the one exception and takes no wash; a submodule is pinned rather than browsable, and a wash with no click target is a false affordance.
