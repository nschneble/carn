...
- **Hit area** — Each cell's own link fills its own cell, so a three-column row is three separately focusable targets covering the row end to end, with a `--sunk` background on the whole `<tr>` on hover and focus. There is no row-wide overlay: it would swallow the subject and the age, which are links of their own. **Live everywhere a row leads somewhere**, which 1e made true of the file tree — file rows link to `/r/:repo/blob/:rev/*`, directory rows to `/r/:repo/tree/:rev/*`. A gitlink row is the one exception and takes no wash; a submodule is pinned rather than browsable, and a wash with no click target is a false affordance.
